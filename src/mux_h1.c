...
static size_t h1_init_ff(struct stconn *sc, struct buffer *input, size_t count, int may_splice)
{
	struct h1s *h1s = __sc_mux_strm(sc);
	struct h1c *h1c = h1s->h1c;
	size_t ret = 0;
...
static void h1_done_ff(struct stconn *sc)
{
	...
	if (sd->iobuf.pipe) {
#if defined(USE_LINUX_SPLICE)
		total = h1c->conn->xprt->snd_pipe(h1c->conn, h1c->conn->xprt_ctx, sd->iobuf.pipe);
		if (total > 0)
		...
		sd->iobuf.offset = 0;
		sd->iobuf.data = 0;
		// TODO: Be sure it is necessary !!!!
		if (b_data(&h1c->obuf))
			h1c->conn->mux->subscribe(sc, SUB_RETRY_SEND, &sc->wait_event);
	}
...
static int h1_resume_fastfwd(struct stconn *sc, unsigned int flags)
{
	...
	if (sd->iobuf.pipe) {
#if defined(USE_LINUX_SPLICE)
		ret = h1c->conn->xprt->snd_pipe(h1c->conn, h1c->conn->xprt_ctx, sd->iobuf.pipe);
		if (ret > 0)
