...
static size_t h2_init_ff(struct stconn *sc, struct buffer *input, size_t count, int may_splice)
{
	struct h2s *h2s = __sc_mux_strm(sc);
	struct h2c *h2c = h2s->h2c;
	struct buffer *mbuf;
 ...
 retry:
	...
	if ((h2s->sd->iobuf.flags & IOBUF_FL_FF_BLOCKED) && !(sc->wait_event.events & SUB_RETRY_SEND))
		h2c->conn->mux->subscribe(sc, SUB_RETRY_SEND, &sc->wait_event);
	TRACE_LEAVE(H2_EV_H2S_SEND|H2_EV_STRM_SEND, h2s->h2c->conn, h2s);
	return ret;
}
...
static void h2_done_ff(struct stconn *sc)
{
	...
	if (!mbuf)
		goto end;
	head = b_peek(mbuf, b_data(mbuf) - sd->iobuf.data /* - sd->iobuf.offset */);

	// FIXME: Set ES and switch stream state
	{
		struct xref *peer;
		struct sedesc *sdo;
		...
		if (se_fl_test(sdo, SE_FL_EOI)) {
			...
			h2s->flags |= H2_SF_ES_SENT;
			TRACE_PROTO("ES flag set on outgoing frame", H2_EV_H2S_SEND|H2_EV_STRM_SEND, h2c->conn, h2s);
		}
	}

	if (!sd->iobuf.data && !(h2s->flags & H2_SF_ES_SENT)) {
		if (mbuf && !b_data(mbuf))
			h2_release_mbuf(h2c);
		goto end;
	}
